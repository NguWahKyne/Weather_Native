...
export default function HomeScreen(){
    ...
    return(
        <View className="flex-1 relative" >
            ...
            <Image blurRadius={30} source={require('../assets/images/max1.png')} className="absolute h-full w-full" />
            {
                loading? (
                ...
                <View style={{height:'7%'}} className="mx-4 relative z-50">
                    <View className="flex-row justify-end items-center rounded-full" style={{backgroundColor: showSearch ? theme.bgWhite(0.2) : 'transparent'}} >
                        ...
                        showSearch ? (
                            ...
                            className="pl-6 h-30 flex-1 text-base text-white" />
                        ) : null
                        }
                                    ...
                                    locations.map((loc,index)=>{
                                        ...
                                        return (
                                           <TouchableOpacity onPress={()=>handleLocation(loc)} key={index} className={"flex-row items-center border-0 p-3 px-4 mb-1" +borderClass}>
                                                <MapPinIcon size="25" color="lightgray" />
                ...
                <View className="mx-4 flex justify-around flex-1 mb-2">
                    {/* location  */}
                     <Text className="text-white text-2xl text-center font-bold" >
                       {location?.name} ,
                        <Text className="text-gray-300 text-lg text-center font-semibold" >
                       {""+ location?.country}
                     </Text>
                     </Text>
                     ...
                     <View className="space-y-2">
                        <Text className="text-center text-white text-bold text-6xl ml-5">
                            {current?.temp_c} &#176;
                        </Text>
                        <Text className="text-center text-white tracking-widest text-xl ml-5">
                    ...
                    showsHorizontalScrollIndicator={false} >
                        {
                            weather?.forecast?.forecastday?.map((item,index)=>{
                                ...
                                return(
                                    <View key={index} className="flex justify-center items-center w-24 rounded-3xl py-3 space-y-2 m-2" style={{backgroundColor:theme.bgWhite(0.15)}}>
                        ...
                        {/* <View className="flex justify-center items-center w-24 rounded-3xl py-3 space-y-2" style={{backgroundColor:theme.bgWhite(0.15)}}>
